...
# Gemini (via LlamaIndex)
try:
    from llama_index.llms.gemini import Gemini as LlamaIndexGemini
    LLAMA_INDEX_AVAILABLE = True
except ImportError:
    LLAMA_INDEX_AVAILABLE = False
    print("LlamaIndex Gemini not available, trying direct Google Generative AI")

# Direct Google Generative AI (fallback)
try:
...
class GeminiGenerator:
    ...
    def configure(self, api_key: str):
        ...
        if api_key:
            ...
            print(f"API key set in environment")
        else:
            print(f"No API key provided")
        
        print(f"Attempting to create LlamaIndexGemini with model: '{self.model_name}'")
        
        try:
            print(f"About to create LlamaIndexGemini...")
            print(f"Model name being passed: '{self.model_name}'")
            print(f"Model name type: {type(self.model_name)}")
            print(f"Model name length: {len(self.model_name)}")
            
            # Try to create the LLM
            self.llm = LlamaIndexGemini(model=self.model_name)
            print(f"Successfully configured Gemini model: {self.model_name}")
            return True
        except Exception as e:
            print(f"Error configuring Gemini model '{self.model_name}': {e}")
            print(f"Full error details: {type(e).__name__}: {str(e)}")
            
            # Try to get more info about the error
            if "models/gemini-pro" in str(e):
                print(f"DETECTED: The error mentions 'models/gemini-pro' but we're using '{self.model_name}'")
                print(f"This suggests LlamaIndex is overriding our model name!")
                print(f"Trying direct Google Generative AI approach...")
                
                # Try direct Google Generative AI as fallback
                if GOOGLE_GENAI_AVAILABLE:
                    try:
                        print(f"Attempting direct Google Generative AI with model: {self.model_name}")
                        ...
                        print(f"Direct Google Generative AI successful with model: {self.model_name}")
                        ...
                        print(f"Direct Google Generative AI also failed: {direct_e}")
                        return False
                else:
                    print(f"Direct Google Generative AI not available")
                    return False
    ...
    def generate(self, question, hits, max_tokens=None):
        ...
        if self.llm is None and not self.use_direct:
            self.llm = LlamaIndexGemini(model=self.model_name)

        prompt = self._prompt(question, hits)

        try:
            if self.use_direct and self.direct_model:
                # Use direct Google Generative AI
                print(f"Using direct Google Generative AI model: {self.model_name}")
                response = self.direct_model.generate_content(prompt)
                txt = (response.text or "").strip()
            else:
                # Use LlamaIndex wrapper
                print(f"Using LlamaIndex Gemini model: {self.model_name}")
